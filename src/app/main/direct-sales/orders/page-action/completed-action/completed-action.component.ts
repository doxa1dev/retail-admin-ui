...
@Component({
  selector: 'app-completed-action',
  templateUrl: './completed-action.component.html',
  styleUrls: ['./completed-action.component.scss']
})
export class CompletedActionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._orderService.checkOrderStatus(this.order_id).subscribe(data=>{
      this._orderService.changeOrderPage(data , this.order_id , this.page, 'COMPLETED')
    })
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        this._orderService.getOrderbyOrderId(orderId).subscribe(response => {
          ...
          this.fullOrder = this._orderService.renderDataOrder(response,true);
  ...
  back() {
    this.router.navigate(["/direct-sales/orders"], { state: { selectTab: 6 , page: this.page } });
  }
