...
@Component({
  selector: 'app-to-unbox-action',
  templateUrl: './to-unbox-action.component.html',
  styleUrls: ['./to-unbox-action.component.scss']
})
export class ToUnboxActionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._orderService.checkOrderStatus(this.order_id).subscribe(data=>{
      this._orderService.changeOrderPage(data , this.order_id , this.page , 'TO_UNBOX')
    })
  ...
  async navigateToHost() {
    ...
    if(dataCheck === 1){
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
        width: '700px',
        data: { message: 'Are you sure you want to Mark As Unboxed?', type: "APPROVED" }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result === true) {
          let url = this.fullOrder.need_host ? 'direct-sales/orders/to-host' : 'direct-sales/orders/completed';
          // this.router.navigate([url], { queryParams: { id: this.order } });
          // this.updateStatusOrder(this.order_id, 'TO_HOST');
          this._orderService.updateStatusOfOrder(this.order_id, 'TO_HOST').subscribe(data=>{
            if(data.code === 200){
              this.router.navigate([url], { queryParams: { id: this.order } });
            }
          });
  ...
  async noUnboxed() {
    ...
    if(dataCheck === 1){
      const dialogRef = this.dialog.open(DialogReasonComponent, {
        width: '500px',
        data: { message: 'Not Unboxed Reasons:', orderId: this.order_id }
      });
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result === true) {
          ...
          let url = this.fullOrder.need_host ? 'direct-sales/orders/to-host' : 'direct-sales/orders/completed';
          this.router.navigate([url], { queryParams: { id: this.order } });
          // this.router.navigate(['direct-sales/orders/to-host'], { queryParams: { id: this.order } });
          // this.updateStatusOrder(this.order_id, 'TO_HOST');
        } else {
          dialogRef.close();
        }
  ...
  back() {
    this.router.navigate(["/direct-sales/orders"], { state: { selectTab: 4 , page : this.page } });
  }
