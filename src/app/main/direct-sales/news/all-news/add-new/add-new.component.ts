...
export class AddNewComponent implements OnInit {
  ...
  createNew(){
    ...
    //   width: '500px',
    //   data: { message: 'Do you wish to create the news?' }
    // });
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      width: '500px',
      data: { message: 'Do you wish to create the news?', type: "APPROVED" }
    });
    dialogRef.afterClosed().subscribe(result =>
    {
      if (result === true)
      {
        ...
        this.newsService.createNews(formProduct).subscribe(data =>
        {
          if (data.code == 200)
          {
